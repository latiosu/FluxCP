...
class PaymentNotifyRequest
{
	...
	/**
	 * PayPal's IPN variables organized into a Config instance.
	 *
	 * @access public
	 * @var Config
	 */
	...
	{
		$this->ppLogFile = new LogFile(FLUX_DATA_DIR . '/logs/paypal.log');
		...
		$this->ipnVariables = new Config($ipnPostVars);
	...
	public function process()
	{
		...
		if ((in_array($received_from, $allowed_hosts) || $this->is_trusted_ip($received_from)) && $this->verify()) {
			...
			if (!in_array($receiverEmail, $accountEmails)) {
				...
				$customData = new Config($customArray);
				...
				if ($paymentStatus == 'Completed') {
					...
					if ($banStatuses instanceOf Config) {
	 ...
	 *
	 * @param LoginAthenaGroup $servGroup
	 * @param string $accountID
	 * @param string $serverName
	 * @access private
	 */
	private function logToPayPalTable(LoginAthenaGroup $servGroup, $accountID, $serverName, $trusted, $credits = 0)
	{
		if ($this->txnIsValid) {
			$holdUntil = null;
			if (!$trusted) {

...
class Template
{
		...
		if (file_exists($this->themePath . '/' . $this->themeName . '/manifest.php')) {
			...
			if (!empty($manifest['inherit'])) {
				...
				if (in_array($manifest['inherit'], self::$themeLoaded)) {
					throw new Error('Circular dependencies in themes : ' . implode(' -> ', self::$themeLoaded) . ' -> ' . $manifest['inherit']);
				}

				$config->set('themeName', $manifest['inherit']);
				self::$themeLoaded[] = $manifest['inherit'];
				$this->parentTemplate = new Template($config);
			}
		}
	...
	public function render(array $dataArr = array())
	{
		...
		if (Flux::config('OutputCleanHTML')) {
			...
			$tidyIgnore = false;
			if (($tidyIgnores = Flux::config('TidyIgnore')) instanceOf Config) {
	...
	public function getMenuItems($adminMenus = false)
	{
		...
		$defaultAction = Dispatcher::getInstance()->defaultAction;
		$menuItems = Flux::config('MenuItems');
		$allowedItems = array();

		if (!($menuItems instanceOf Config)) {
			return array();
		}
	...
	public function getSubMenuItems($moduleName = null)
	{
		...
		if (!($subMenuItems instanceOf Config) || !(($menus = $subMenuItems->get($moduleName)) instanceOf Config)) {
			return array();
		}
	...
	{
		$paginator = new Paginator($total, $this->url($this->moduleName, $this->actionName, array('_host' => false)), $options);
		return $paginator;
	}
